...
from app.schemas.schemas import QuizResultCreateInSchema, QuizResultCreateSchema, QuizResultUpdateSchema
from app.db.models.models import QuizModel, QuestionModel, OptionModel, CompanyModel, MemberModel, QuizResultModel
...
async def test_pass_quiz_success(
    mock_get_one_option, mock_get_one_member, mock_get_one_company,
    mock_get_one_quiz, mock_add_quiz_result,
    mock_get_one_quiz_result_by_filter,  get_db_fixture, quiz_result_service
):
    ...
    mock_get_one_company.return_value = CompanyModel(id=1, owner_id=1, name="Test Company", description="Test Description")
    ...
    mock_add_quiz_result.return_value = QuizResultModel(id=1, quiz_id=1, company_id=1, score=1.0, user_id=1)

    async for db in get_db_fixture:
        result = await quiz_result_service.pass_quiz(data=quiz_data, user_id=user_id, db=db)
